...
const Card = ({
  ...
  const [active, setActive] = useState(
    label in cartItems && cartItems[label][1] != 0 ? false : true
  );
    ...
    <div className="flex flex-col items-center border border-black w-fit h-[24rem] m-4 rounded-lg shadow-black ">
      ...
      {showButton ? (
        <button
          ...
          } hover:bg-gray-100 text-gray-800 font-semibold py-1 px-4 border border-gray-400 rounded shadow m-2 size-fit text-sm`}
          onClick={() => {
            active
              ? setCartItems({ item: label, quantity: 1, price: price })
              : setCartItems({ item: label, quantity: 0, price: 0 });
